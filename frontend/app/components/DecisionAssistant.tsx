...
function heatColorFrom01(v: number) {
  ...
  if (n < 0.25) return "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300";
  if (n < 0.5) return "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300";
  if (n < 0.75) return "bg-lime-100 text-lime-800 dark:bg-lime-900/30 dark:text-lime-300";
      ...
      {role === "research" && (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          <div className="xl:col-span-1 bg-white dark:bg-gray-800 p-6 rounded shadow space-y-3 transition-colors duration-200">
            ...
            <label className="text-sm text-gray-700 dark:text-gray-300 block">Idea description<HelpTip text="Write 1-3 sentences about the project scope and goal." />
              <textarea className={inputStyle} rows={4} value={ideaText} onChange={(e) => setIdeaText(e.target.value)} />
            </label>
            <label className="text-sm text-gray-700 dark:text-gray-300 block">CMU campus<HelpTip text="Campus affects historical likelihood and capacity signals." />
              <select className={inputStyle} value={campusCode} onChange={(e) => setCampusCode(e.target.value)}>
                {campuses.map((c) => (<option key={c.code} value={c.code}>{c.label}</option>))}
              </select>
            </label>
            <label className="text-sm text-gray-700 dark:text-gray-300 block">Research field<HelpTip text="Pick the closest FOR4 field; this drives model matching." />
              <select className={inputStyle} value={fieldCode} onChange={(e) => setFieldCode(e.target.value)}>
                {fields.map((f) => (<option key={f.code} value={f.code}>{f.name}</option>))}
              </select>
            </label>
            <label className="text-sm text-gray-700 dark:text-gray-300 block">Project length (months)
              <input className={inputStyle} type="number" min={6} max={60} value={lengthMonths} onChange={(e) => setLengthMonths(parseInputNumber(e.target.value))} />
            </label>
            <label className="text-sm text-gray-700 dark:text-gray-300 block">Project start date<HelpTip text="Used to increase urgency for near-term projects in Admin Inbox." />
              <input className={inputStyle} type="date" value={projectStartDate} onChange={(e) => setProjectStartDate(e.target.value)} />
            </label>
            <div className="text-sm block">
              <p className="text-gray-700 dark:text-gray-300">Budget mode<HelpTip text="Use generated estimate or override with your own requested budget." /></p>
              <div className="flex gap-3 mt-1 text-gray-900 dark:text-white">
                <label className="flex items-center gap-1"><input type="radio" checked={budgetMode === "auto"} onChange={() => setBudgetMode("auto")} /> Generated</label>
                <label className="flex items-center gap-1"><input type="radio" checked={budgetMode === "manual"} onChange={() => setBudgetMode("manual")} /> Manual</label>
              </div>
            </div>
            {budgetMode === "manual" && (
              <label className="text-sm text-gray-700 dark:text-gray-300 block">Manual requested budget (USD)
                <input className={inputStyle} type="number" min={0} value={requestedBudget} onChange={(e) => setRequestedBudget(parseInputNumber(e.target.value))} />
              </label>
            )}
            <label className="text-sm text-gray-700 dark:text-gray-300 block">Already received (USD)
              <input className={inputStyle} type="number" min={0} value={alreadyReceived} onChange={(e) => setAlreadyReceived(parseInputNumber(e.target.value))} />
            </label>
            <div className="border dark:border-gray-600 rounded p-2 space-y-2">
              <p className="text-xs font-semibold text-gray-600 dark:text-gray-400">Subject allocations<HelpTip text="Optional breakdown by subject area; helps constrain budget recommendations." /></p>
              {subjectAllocations.map((a, idx) => (
                <div key={idx} className="grid grid-cols-2 gap-2">
                  <input className="border dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded p-1 text-xs" placeholder="Subject area" value={a.area} onChange={(e) => updateAllocation(idx, { area: e.target.value })} />
                  <input className="border dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded p-1 text-xs" type="number" min={0} placeholder="USD" value={a.amount} onChange={(e) => updateAllocation(idx, { amount: Number(e.target.value || 0) })} />
                </div>
              ))}
              <button className="text-xs border dark:border-gray-600 rounded px-2 py-1 text-gray-700 dark:text-gray-300" onClick={() => setSubjectAllocations((prev) => [...prev, { area: "", amount: 0 }])}>+ Add row</button>
            </div>
            <div className="flex gap-2">
              ...
              <button className="flex-1 border dark:border-gray-600 text-gray-900 dark:text-white rounded p-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700" onClick={saveCurrentIdea} disabled={!fieldCode || !campusCode}>Save Idea</button>
            </div>
            {saveNotice && <p className="text-xs text-green-700 dark:text-green-400">{saveNotice}</p>}
          </div>

          <div className="xl:col-span-2 grid grid-cols-1 gap-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="bg-white dark:bg-gray-800 p-4 rounded shadow"><p className="text-xs text-gray-500 dark:text-gray-400">Field</p><p className="font-semibold">{selectedField?.name || "-"}</p></div>
              <div className="bg-white dark:bg-gray-800 p-4 rounded shadow"><p className="text-xs text-gray-500 dark:text-gray-400">Opportunity</p><p className="font-semibold">{(researcherResult?.summary.opportunity_score || selectedField?.opportunity_score || 0).toFixed(3)}</p></div>
              <div className="bg-white dark:bg-gray-800 p-4 rounded shadow"><p className="text-xs text-gray-500 dark:text-gray-400">Selected Budget</p><p className="font-semibold">{formatCompactUsd(researcherResult?.budget.selected_budget || 0)}</p></div>
              <div className="bg-white dark:bg-gray-800 p-4 rounded shadow"><p className="text-xs text-gray-500 dark:text-gray-400">Remaining Need</p><p className="font-semibold">{formatCompactUsd(researcherResult?.budget.remaining_need || 0)}</p></div>
            </div>

            {researcherResult && (
              <>
                <div className="bg-white dark:bg-gray-800 p-6 rounded shadow overflow-hidden">
                    ...
                    <table className="w-full text-sm border dark:border-gray-600">
                      <thead className="bg-gray-50 dark:bg-gray-900/50">
                        <tr className="text-left">
                          <th className="p-2 border dark:border-gray-600">Funder</th>
                          <th className="p-2 border dark:border-gray-600">Win Prob</th>
                          <th className="p-2 border dark:border-gray-600">Fit</th>
                          <th className="p-2 border dark:border-gray-600">Expected Award</th>
                        </tr>
                      </thead>
                      <tbody>
                        {researcherResult.top_funders.map((f) => {
                          ...
                          return (
                            <tr key={f.funder_name}>
                              <td className="p-2 border dark:border-gray-600 font-medium">{f.funder_name}</td>
                              <td
                                className={`p-2 border dark:border-gray-600 ${heatColorFrom01(Number(f.cmu_win_probability || 0))}`}
                              ...
                              <td
                                className={`p-2 border dark:border-gray-600 ${heatColorFrom01(Number(f.fit_score || 0))}`}
                              ...
                              <td
                                className={`p-2 border dark:border-gray-600 ${heatColorFrom01(awardNorm)}`}
                ...
                <div className="bg-white dark:bg-gray-800 p-6 rounded shadow">
                  ...
                  {researcherResult.likelihood_context?.funder_data_mode !== "field_observed" && (
                    <div className="mb-3 rounded border border-amber-300 dark:border-amber-700/50 bg-amber-50 dark:bg-amber-900/20 p-2 text-xs text-amber-800 dark:text-amber-200">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 grid grid-cols-1 gap-4">
            <div className="bg-white p-6 rounded shadow">
              ...
              <p className="text-xs text-gray-600 dark:text-gray-400 mb-2">Urgency is shown as a percent relative to the highest-urgency submission in this inbox.</p>
              {adminInboxItems.length === 0 ? (
                <p className="text-sm text-gray-600 dark:text-gray-400">No saved ideas yet.</p>
              ) : (
